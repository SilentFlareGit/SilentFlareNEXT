...
The Subjects, Risk factors, Geography, and Sites views operate on live Shield data. Geography provides the complete ISO 3166 country and subdivision catalog instead of limiting configuration to previously observed locations. Country and regional restrictions require an audit reason; known matching Account and IP roots immediately receive a policy-owned effective score floor of 100, and newly observed matching roots receive the same control. Removing the restriction revokes only that policy floor, revealing each root's preserved underlying score and retaining append-only `GEOGRAPHY_RESTRICTION_APPLIED` and `GEOGRAPHY_RESTRICTION_REVOKED` ledger records. Enforcement still follows the selected site mode: observe records the decision, while enforce denies it.
