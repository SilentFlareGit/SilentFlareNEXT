...
<section class="shield-workspace">
	...
	<nav class="security-tabs" aria-label="Security views">
		...
		<button class:active={activeView === "entities"} onclick={() => (activeView = "entities")}><Icon icon="material-symbols:hub-outline-rounded"/>Entities</button>
	...
	{#if loading && !dashboard}<div class="loading"><span></span>Loading security telemetry...</div>{:else if dashboard}
		...
		{:else if activeView === "operations"}
			...
			<section class="panel"><div class="panel-head"><div><h2>Automation runtime</h2><p>Queued signals, failures, and recent worker runs</p></div><span>{dashboard.automation.queuedSignals} queued / {dashboard.automation.failedSignals} failed</span></div><div class="compact-list">{#each dashboard.automation.runs as run}<div><div><strong>{run.jobName}</strong><small>{run.status} / {run.processedCount} changes / {formatTime(run.startedAt)}</small></div><span class={`review severity-${run.status === "failed" ? "critical" : "info"}`}>{run.status}</span></div>{:else}<div><div><strong>No worker runs recorded</strong><small>The worker will report after its next automation cycle.</small></div></div>{/each}</div></section>
...
@media(min-width:1024px){.metric-grid{grid-template-columns:repeat(6,1fr)}.metric-grid>div,.metric-grid>div:last-child{grid-column:auto;border-bottom:0}.two-column{grid-template-columns:minmax(0,1fr) minmax(0,1fr)}.geo-create{grid-template-columns:repeat(4,minmax(0,1fr)) auto}.alert-config{grid-template-columns:repeat(5,minmax(0,1fr)) auto auto}}
