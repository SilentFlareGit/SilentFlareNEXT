		...
		<section class="identity-shell">
			...
			<div class="identity-form-column">
				<section class="identity-form">
					...
					<div class="identity-form-content">{@render children()}</div>
				</section>
...
<style>
	...
	.theme-command { position: absolute; z-index: 2; top: 1rem; right: 1rem; }
	.identity-main { width: min(100%, 80rem); margin: 0 auto; }
	.identity-shell { display: grid; gap: 1.5rem; padding: 1rem 0; }
	.identity-story { order: 1; min-width: 0; display: flex; flex-direction: column; gap: 1rem; padding: 0.5rem 0; }
	...
	.identity-form-column { order: -1; min-width: 0; }
	.identity-form { position: relative; min-width: 0; min-height: 28rem; display: flex; align-items: center; padding: 4.5rem 1.25rem 1.25rem; border: 1px solid var(--sf-border); border-radius: var(--sf-radius-lg); background: var(--sf-surface); box-shadow: 0 0.75rem 2.5rem rgba(28, 53, 79, 0.07); }
	.identity-form-content { width: 100%; min-width: 0; }
	...
	@media (min-width: 768px) {
		...
		.identity-form { padding: 4.75rem 2.5rem 2.5rem; }
	}
	@media (min-width: 1024px) {
		...
		.identity-story { order: 0; min-height: 29rem; justify-content: center; padding: 2rem 0; }
		.identity-form-column { order: 0; }
		.identity-form { padding: 4.75rem 3rem 3rem; }
